        ...
        if self.dimension == 0:
            for aloop in self.loops:
                for apacket in aloop.recursive_iter():
                    ...
                    yield dict(top_items + list(apacket.items()))
        else:
                ...
                if self.loops:
                  for aloop in self.loops:
                    for apacket in aloop.recursive_iter(coord=coord+[i]):
                        ...
                        yield dict(kvpairs + list(apacket.items()))
    ...
    def AddLoopItem(self,data,precheck=False,maxlength=-1):
        ...
        if isinstance(data[0],(TupleType,ListType)):
           ...
           if data[1] == [] or self.get_dim(data[1])[0] == self.dimension:
               ...
               raise StarLengthError("input data dim %d != required dim %d: %s %s" % (self.get_dim(data[1])[0],self.dimension,data[0],repr(data[1])))
...
class BlockCollection:
    ...
    def merge(self,new_bc,mode="strict",single_block=[],
        ...
        if match_att:
            ...
            block_to_item = [self[a].get(match_att,None) for a in list(self.keys())]
            # print `block_to_item`
        for key in new_keys:
